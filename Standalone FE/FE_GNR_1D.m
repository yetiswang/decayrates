...
function [FE] = FE_GNR_1D( height, diameter, metal, enei_field, enei_dipole )
...
plot( d_BEM, ee, d_BEM, Q_avg./0.01, d_BEM,FE(1,:)  )
xlabel('d (nm)')
...
for i = 1 : length(QY)
    ...
    plot( d_BEM, I_ph(i,:).*tau_bm )
    hold on
    hline(I_ph_bg(i).*tau_bm,':')
    
end
legend( 'QY 0.01','QY 0.05','QY 0.25','QY 0.75' )
set(gca,'YScale','log')
