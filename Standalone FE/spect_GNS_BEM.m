%% BEM retarded calculation of nanosphere scattering spectrum
% The particle is set by default to lie along the x axis, and plane wave exctation is
% set by default polarized along the x axis. 
%       Input :   diamter: diameter of the nanosphere
...
function [ sca, fit, Lorentz ] = spect_GNS_BEM( epstab, diameter, enei, ztab )
...
ndir = sprintf('%d nm GNSs',diameter);
...
if ztab == 'NaN'
    ...
    if ~exist( 'greentab', 'var' ) || ~ greentab.ismember( layer, enei, p )
        ...
        greentab = set( greentab, linspace( 200, 950, 5 ), op );
    end
...
%% LORENTZIAN FIT
[ fit, Lorentz, Lorentz_wl ] = LorentzFitYW( sca, enei, ndir,'sca' ) ;